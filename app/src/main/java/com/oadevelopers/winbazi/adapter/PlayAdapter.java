...
import com.oadevelopers.winbazi.activity.RoomAuthActivity;
import com.oadevelopers.winbazi.session.SessionManager;
import com.oadevelopers.winbazi.bottomsheet.BottomSheetMyEntries;
import com.oadevelopers.winbazi.views.ViewAnimation;
import com.squareup.picasso.Picasso;

import java.util.HashMap;
import java.util.List;

import com.oadevelopers.winbazi.bottomsheet.BottomSheetPricePool;
import com.oadevelopers.winbazi.R;
import com.oadevelopers.winbazi.activity.JoiningMatchActivity;
import com.oadevelopers.winbazi.activity.PlayDetailsActivity;
...
public class PlayAdapter extends RecyclerView.Adapter<PlayAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final PlayAdapter.ViewHolder holder, int position) {
        ...
        if (!playPojo.getImage().equals("null")) {
            ...
            Picasso.get().load(Config.FILE_PATH_URL + playPojo.getImage().replace(" ", "%20")).resize(500, 250).into(holder.topImage);
        } else {
        ...
        if (playPojo.getTotal_joined() >= playPojo.getRoom_size()) {
            ...
            holder.lyt_join.setEnabled(false);
            holder.lyt_join.setClickable(false);
        } else {
            ...
            holder.lyt_join.setEnabled(true);
            holder.lyt_join.setClickable(true);
        }
        ...
        try {
            if (playPojo.getIs_cancel().equals("1")) {
                ...
                holder.joinTv.setText("CANCELED");
                ...
                holder.lyt_join.setEnabled(true);
                holder.lyt_join.setClickable(true);
            } else if (playPojo.getUser_joined().equals("1") || playPojo.getUser_joined().equals("2") || playPojo.getUser_joined().equals("3") || playPojo.getUser_joined().equals("4") && playPojo.getTotal_joined() < playPojo.getRoom_size()) {
                holder.joinTv.setText("My Entries");
                holder.joinBtn.setText("View More");
                holder.lyt_join.setEnabled(true);
                holder.lyt_join.setClickable(true);
            } else {
                holder.spot.setTextColor(Color.parseColor("#ff0000"));
        ...
        holder.back_drop.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                rotate = !rotate;
                if (rotate) {
                    ViewAnimation.showIn(holder.lyt_match);
                    ViewAnimation.showIn(holder.lyt_room);
                    ViewAnimation.showIn(holder.lyt_join);
                    holder.img_drop.setVisibility(View.VISIBLE);
                    holder.back_drop.setVisibility(View.VISIBLE);
                    holder.lyt_join.setVisibility(View.VISIBLE);
                    holder.joinBtn.setText("Cancel");
                } else {
                    ViewAnimation.showOut(holder.lyt_match);
                    ViewAnimation.showOut(holder.lyt_room);
                    ViewAnimation.showOut(holder.lyt_join);
                    holder.img_drop.setVisibility(View.GONE);
                    holder.back_drop.setVisibility(View.GONE);
                    holder.lyt_join.setVisibility(View.GONE);
                    holder.joinBtn.setText("View More");
                }
            }
        });
        ...
        holder.lyt_join.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                new BottomSheetMyEntries(playPojo.getId(), id, password).show(((FragmentActivity) context).getSupportFragmentManager(), "myBidsBottomSheet");
        ...
        holder.lyt_match.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (new ExtraOperations().haveNetworkConnection(context)) {
        ...
        holder.lyt_room.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(context, RoomAuthActivity.class);
                intent.putExtra("EntryFee_KEY", playPojo.getEntry_fee());
    ...
    class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View itemView) {
            ...
            this.watchBtn = (Button) itemView.findViewById(R.id.watchbtn);
